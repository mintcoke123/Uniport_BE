...
public class ChatRoomController {
    ...
    public ResponseEntity<Map<String, Object>> renameChatRoom(
            @PathVariable Long roomId,
            @RequestBody(required = false) RenameChatRoomRequest request,
            @AuthenticationPrincipal FirebaseAuthenticatedUser principal,
            @RequestHeader(value = "Authorization", required = false) String authorization) {
        User user = currentUserResolver.resolveRequired(principal, authorization);
        String name = request != null ? request.name() : null;
        return ResponseEntity.ok(chatRoomService.renameChatRoom(roomId, user, name));
    }

    public record RenameChatRoomRequest(String name) {
    }
}
